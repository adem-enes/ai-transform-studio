...
/**
 * The response goes out within Magic Hour's 10 s budget; a `*.completed`
 * event's copy into Cloudinary then runs in `after()`, which shares this
 * limit. The Cloudinary SDK gives a video copy up to 180 s
...
export const POST = createWebhookHandler({
  ...
  nowMs: () => Date.now(),
  handle: (event) => handleWebhookEvent(event, { ...appDeps(), defer: after }),
  logger,
});
